...
from integrations import port2port, cybercellar, winecellar, vinoteque
...
@shared_task(base=NotifyFailTask)
def winecellar_update_task():
    winecellar.update_all()


@shared_task(base=NotifyFailTask)
def vinoteque_update_task():
    vinoteque.update_all()
